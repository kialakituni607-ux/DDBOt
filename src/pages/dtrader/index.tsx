...
const DTrader = observer(() => {
    ...
    const [symbolLabel, setSymbolLabel] = useState('Volatility 100 (1s) Index');
    ...
    const [digitStats, setDigitStats] = useState<number[]>(Array(10).fill(0));
    const [buying, setBuying] = useState(false);
    const [message, setMessage] = useState('');
    const svgRef = useRef<SVGSVGElement>(null);
    const priceHistory = useRef<number[]>([]);
    const tickSub = useRef<any>(null);
    const proposalSub = useRef<any>(null);
    const proposalId = useRef<string | null>(null);

    const drawChart = useCallback((prices: number[]) => {
        ...
        svg.innerHTML = `
            ...
            <path d="${path} L${lx},${h} L0,${h} Z" fill="url(#g)"/>
            <path d="${path}" fill="none" stroke="${col}" stroke-width="1.5"/>
            <circle cx="${lx}" cy="${ly}" r="4" fill="${col}"/>
            <line x1="${lx}" y1="0" x2="${lx}" y2="${h}" stroke="#444" stroke-width="1" stroke-dasharray="3"/>
        `;
    }, []);

    const subscribeTicks = useCallback((sym: string) => {
        if (tickSub.current) { tickSub.current.unsubscribe?.(); tickSub.current = null; }
        priceHistory.current = [];
        setDigitStats(Array(10).fill(0));
        const digits: number[] = [];
        tickSub.current = api_base.api.onMessage().subscribe(({ data }: any) => {
    ...
    const getProposal = useCallback(() => {
        if (proposalSub.current) { proposalSub.current.unsubscribe?.(); proposalSub.current = null; }
        const tradeType = TRADE_TYPES[activeTab];
        const choice = tradeType.choices[activeChoice];
        const contractType = CONTRACT_TYPE_MAP[tradeType.label]?.[choice];
        ...
        const proposal: any = {
            ...
            ...(isOTP ? { underlying_symbol: symbol } : { symbol }),
        };
        if (tradeType.hasDigit) proposal.barrier = digit;
        proposalSub.current = api_base.api.onMessage().subscribe(({ data }: any) => {
    ...
    useEffect(() => { subscribeTicks(symbol); }, [symbol]);
    useEffect(() => { getProposal(); }, [activeTab, activeChoice, symbol, stake, duration, durationUnit, digit]);
    ...
    const handleBuy = async () => {
        ...
        setMessage('');
        try {
            const res: any = await api_base.api.send({ buy: proposalId.current, price: stake });
            if (res.buy) {
                setMessage(`✅ Bought! Contract ID: ${res.buy.contract_id}`);
            } else if (res.error) {
            ...
            <div className='dtrader-body'>
                <div className='dtrader-left'>
                    <div className='dtrader-market-bar'>
                        <div>
                            ...
                            <div className='dtrader-mprice'>{price?.toFixed(2) ?? '---'}</div>
                        </div>
                        <select className='dtrader-select' value={symbol} onChange={e => {
                            setSymbol(e.target.value);
                            setSymbolLabel(SYMBOLS.find(s => s.value === e.target.value)?.label ?? '');
                        }}>
                ...
                <div className='dtrader-form'>
                            ...
                            <div className='dtrader-digit-grid'>
                                {Array.from({ length: 10 }, (_, i) => (
                                    <div key={i} className={`dtrader-digit${digit === i ? ' active' : ''}`} onClick={() => setDigit(i)}>
                                        ...
                                        <span className={`dtrader-digit-pct${digitStats[i] === Math.min(...digitStats.filter(Boolean)) ? ' hot' : ''}`}>{digitStats[i] || '0'}%</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <button className='dtrader-buy-btn' onClick={handleBuy} disabled={buying || !proposalId.current}>
                        ...
                        <div className='dtrader-buy-sub'>Payout {payout?.toFixed(2) ?? '---'} USD</div>
                    </button>
                </div>
            </div>
        </div>
